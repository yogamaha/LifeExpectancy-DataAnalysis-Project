...
# TO DO Yoga, I think it's great you found these transformations that work. For prediction they are superior to the models I found. Can you explain a little how you got them. I see the code, but I don't understand what prompted you to make these transformations. This would be very informative for the reader. 
...
|Combined AIC| `r rmse(combined_model_aic)` | `r rmse(combined_model_aic, FALSE)` | `r length(coef(trained_model_trans)) - 1`
|Combined BIC   | `r rmse(combined_model_bic)`   | `r rmse(combined_model_bic, FALSE)`   |`r length(coef(trained_model_add)) - 1`
...
### TO DO QUESTION TO YOGA: Can we explain the rationale between the transformations you used (e.g. Income.composition.of.resources (^2) and Schooling (sqrt))? I think it's great that you found a model that is able to improve on the combined models that derive from the categories, but I myself and wondering what exactly these transformations do.

### I'm thinking that for explainability, the combined models are decent. One approach may be to say that for explainability, they offer a clear description of what is going on with life expectancy. 

### However, if you are interested in prediction, the transformation models work better. I think if you add comments on why you made those transformations, then we can wrap this up. Let's make sure it conforms to the rubric. 

### Has Zoheb contacted you at all? Should we complete it without him? I haven't heard a word from him. I am going to be traveling on Saturday and Sunday and may not have Internet, so I need to be done with this 24 hours from now. I also need to complete my evaluation of other models then too. 

### FINAL TO DO:
- explain the rationale behind the transformations. if there is none, simply say they were found by trial and error
- explain why we omitted year from the computations
- explain clearly why country was omitted (you may have done that, but I didn't see it)
