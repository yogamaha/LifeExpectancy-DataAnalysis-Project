...
**Lifestyle**
```{r}
lifestyle <- subset(train_df, select = c(Life.expectancy, Alcohol, Adult.Mortality, infant.deaths, under.five.deaths, Schooling, Population))
...
lifestyle_model <- lm(Life.expectancy ~ Alcohol + Adult.Mortality + infant.deaths + under.five.deaths + Schooling, data = train_df)
```
**Money Model**
```{r}
...
**Lifestyle**
```{r}
lifestyle_reduced <- lm(Life.expectancy ~ Adult.Mortality + under.five.deaths + Schooling + Population, data = train_df)
...
Again we can see there is a `r (trans_r2 - red_r2)/red_r2`% increase in the Adjusted $R^2$ and while increasing the size of the model by only `r length(coef(money_trans)) - length(coef(money_reduced))` predictors. This is a huge improvement.
...
Here we can see there is a `r (trans_r2 - red_r2)/red_r2`% increase in the Adjusted $R^2$ and while increasing the size of the model by `r length(coef(money_trans)) - length(coef(money_reduced))` predictors. The increase in Adjusted $R^2$ is so small that it might be better to stick with the reduced model

**Lifestyle**
```{r}
lifestyle_trans <- lm(Life.expectancy ~ Adult.Mortality + under.five.deaths + Schooling + Population + sqrt(Schooling) , data = train_df)
...
Here we can see there is a `r (trans_r2 - red_r2)/red_r2`% decrease in the Adjusted $R^2$ and while increasing the size of the model by `r length(coef(money_trans)) - length(coef(money_reduced))` predictor. It would definitely be better to stick with the Reduced Model in this case.
...
aic_model <- step(money_trans,
                       scope = Life.expectancy ~ log_per_cap_GDP + Income.composition.of.resources + I(Income.composition.of.resources^2) + I(Income.composition.of.resources^3) + Measles + Polio + Diphtheria + HIV.AIDS + log(HIV.AIDS) + BMI + thinness..1.19.years + Adult.Mortality + under.five.deaths + Schooling + Population,
                       direction = "both",
                       trace = 0)
summary(aic_model)
```
...
full_model <- lm(Life.expectancy ~ .- Status - GDP - Population + I(Income.composition.of.resources^2) + I(Income.composition.of.resources^3) + log(HIV.AIDS), data = train_df) #We are also removing the GDP and Population Params as we will use the log_per_cap_GDP instead
summary(full_model)
...
Here we can see there is a `r (full_r2 - aic_r2)/aic_r2 * 100`% increase in the Adjusted $R^2$ and also increasing the size of the model by `r length(coef(full_model)) - length(coef(aic_model))` predictors. It would definitely be better to stick with the AIC Model.

#### Adding The Factor Variable

Now that we have created our final model using the numerical predictors, its time to add the categorical predictor. We're going to create 2 more models, one that just adds the Status predictor to the AIC and one that adds an interaction between Status and all the other predictors

```{r}
add_model = lm(Life.expectancy ~ log_per_cap_GDP + Income.composition.of.resources + 
    ...
    Diphtheria + BMI + Population + Status, data = train_df)

int_model = lm(Life.expectancy ~ (log_per_cap_GDP + Income.composition.of.resources + 
    I(Income.composition.of.resources^2) + I(Income.composition.of.resources^3) + 
    log(HIV.AIDS) + Adult.Mortality + HIV.AIDS + under.five.deaths + 
    Diphtheria + BMI + Population) * Status, data = train_df)

add_r2 <- summary(add_model)$adj.r.squared
int_r2 <- summary(int_model)$adj.r.squared

#aic_loocv <- calc_loocv_rmse(aic_model)
#add_loocv <- calc_loocv_rmse(add_model)
#int_loocv <- calc_loocv_rmse(int_model)

anova_aic_add <- anova(aic_model,add_model)[2,"Pr(>F)"]

anova_aic_int <- anova(aic_model,int_model)[2,"Pr(>F)"]

anova_add_int <- anova(add_model,int_model)[2,"Pr(>F)"]
```

|Model        |Adjusted $R^2$ |Anova vs AIC Model|No. Of Predictors             |
|-------------|---------------|------------------|------------------------------|
|AIC          |`r aic_r2`     | NA               |`r length(coef(aic_model))-1` |
|Additive     |`r add_r2`     |`r anova_aic_add` |`r length(coef(add_model))-1` |
|Interactive  |`r int_r2`     |`r anova_aic_int` |`r length(coef(int_model))-1` |
...
|Additive Model        | `r rmse(add_model)` | `r rmse(add_model, FALSE)` | `r length(coef(add_model)) - 1`|
|Interactive Model        | `r rmse(int_model)` | `r rmse(int_model, FALSE)` | `r length(coef(int_model)) - 1`|



Looking at the RMSE we can see that the model with the transformed predictors as it has a lower RMSE for both the training and testing data. It also has less predictors and we prefer smaller models. As for the combined models that were derived by combining "mini category-based" models, although their explainability is clearly preferable given that they employ largely straightforward combinations of variables, they produce slightly higher RMSEs than both the transformed and additive models. In the end, the transformed model, with only 10 predictors, is best.
...
```{r}
bp_aic_val = bptest(aic_model)$p.value
bp_add_val = bptest(add_model)$p.value
bp_int_val = bptest(int_model)$p.value
```
                              
|Model        |BP P-Val |
|-------------|------------------|
|AIC          |`r bp_aic_val`    |
|Additive     |`r bp_add_val`    |
|Interactive  |`r bp_int_val`    |


```{r}
par(mfrow = c(2,2))
plot(fitted(aic_model), resid(aic_model),  col = "darkgreen",
     ylab = "Residuals",
     xlab = "Fitted Values",
     main = "AIC Model")
abline(h = 0, lwd = 3, col = "goldenrod")

plot(fitted(add_model), resid(add_model),  col = "red",
     ylab = "Residuals",
     xlab = "Fitted Values",
     main = "Additive Model")
abline(h = 0, lwd = 3, col = "goldenrod")
...
#par(mfrow = c(1,2))
#Interaction Model
plot(fitted(int_model), resid(int_model),  col = "blue",
     ylab = "Residuals",
     xlab = "Fitted Values",
...
|Model        |Shapiro-Wilks P-Value |
|-------------|----------------------|
|AIC          |`r shapiro.test(resid(aic_model))$p.value`    |
|Additive     |`r shapiro.test(resid(add_model))$p.value`    |
|Interactive  |`r shapiro.test(resid(int_model))$p.value`    |

These values are strange, it appears that the only Model who's P-value improved was the Money model. Let's look at the graphs while comparing them to the reduced models

```{r}
par(mfrow = c(2,2))
qqnorm(resid(aic_model), main = "AIC Model", col = "darkgreen")
qqline(resid(aic_model))

qqnorm(resid(add_model), main = "Additive Model", col = "red")
qqline(resid(add_model))

qqnorm(resid(int_model), main = "Interactive Model", col = "blue")
...
### Summary:
- Final Model with the following predictors is the best fitting model : log_per_cap_GDP, Income.composition.of.resources, HIV.AIDS, Adult.Mortality, under.five.deaths, Population, Diphtheria
